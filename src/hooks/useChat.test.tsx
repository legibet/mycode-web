...
describe("useChat", () => {
  ...
  it("sends inline workspace references as Wails path blocks", async () => {
    appMock.StartChat.mockImplementation((req) =>
      ok({
        run: {
          id: "run-1",
    ...
    expect(appMock.StartChat.mock.calls[0]?.[0].input).toEqual([
  ...
  it("starts and follows a compact run through Wails events", async () => {
    saveActiveSession("/workspace/a", "session-c");
    appMock.ListSessions.mockImplementation(() =>
      ok({ sessions: [{ id: "session-c", title: "Long chat" }] }),
    );
    ...
    appMock.CompactSession.mockImplementation(() =>
      ok({
        run: {
          id: "run-c",
    ...
    expect(appMock.CompactSession).toHaveBeenCalledWith("session-c", {
      provider: "anthropic",
      model: "m1",
    });
    ...
    emitRun("run-c", "session-c", { type: "compact", seq: 1 });
    emitRun("run-c", "session-c", { type: "done", status: "completed" });
